...
local exploit = (syn and not is_sirhurt_closure and not pebc_execute and "Synapse" or getexecutorname and getexecutorname() == "ScriptWare" and "Script-Ware")

local decode = "NULL"
local getcustomasset = "NULL"
local AprilSound;
local date = os.date("*t", os.time())

if exploit == "Synapse" or exploit == "Script-Ware" then
	decode = exploit == "Synapse" and syn.crypt.base64.decode or crypt.base64decode
	getcustomasset = exploit == "Synapse" and getsynasset or getcustomasset
end
...
if decode ~= "NULL" and getcustomasset ~= "NULL" and date.month == 4 and date.day == 1 then
    local SoundData = game:HttpGet("https://thisstuff.xyz/Api/April1st.txt")
    AprilSound = Instance.new("Sound", game:GetService("CoreGui"))

    if not SoundData then
        return;
    end

    writefile("ThisStuffAprilFools.mp3", decode(SoundData))

    AprilSound.Playing = true
    AprilSound.SoundId = getcustomasset("ThisStuffAprilFools.mp3")
    AprilSound.Looped = true
end

UIS.InputBegan:Connect(function(input, gameProcessed)
	if input.KeyCode == (ui_options.toggle_key or Enum.KeyCode.RightControl) then
		if script.Parent then
			if not checks.binding then
				script.Parent.Enabled = not script.Parent.Enabled
				if AprilSound then
					AprilSound.Playing = script.Parent.Enabled == true and true or false
				end
			end
		end
	end
